...
const port = 3000; // defining port address(number) of my web server app where it will listen for incoming requests
...
// Available routes
app.use('/api/auth', require('./routes/auth')); // loads your router from routes/auth.js, mounts it at /api/auth, so any route inside auth.js like: router.post('/', ...) becomes accessible at: POST http://localhost:3000/api/auth/
...
//starting my web server app and listening on the specified port 3000, once my web server is running, logging a msg in the console
app.listen(port, () => {
